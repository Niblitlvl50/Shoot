...
void PathEntity::UpdateMesh()
{
    ...
    const uint32_t position_buffer_data_size = generated_mesh->num_vertices;

    if(!m_position_buffer || m_position_buffer->Size() < position_buffer_data_size)
    {
        m_position_buffer = CreateRenderBuffer(BufferType::DYNAMIC, BufferData::FLOAT, 2, position_buffer_data_size, nullptr);
    }
    m_position_buffer->UpdateData(generated_mesh->positions, 0, position_buffer_data_size);

    if(!m_color_buffer || m_color_buffer->Size() < position_buffer_data_size)
    {
        m_color_buffer = CreateRenderBuffer(BufferType::DYNAMIC, BufferData::FLOAT, 4, position_buffer_data_size, nullptr);
    }
    const std::vector<mono::Color::RGBA> colors(generated_mesh->num_vertices, mono::Color::RED);
    m_color_buffer->UpdateData(colors.data(), 0, position_buffer_data_size);
    ...
    {
        m_index_buffer = CreateElementBuffer(BufferType::DYNAMIC, index_buffer_data_size, nullptr);
    }
