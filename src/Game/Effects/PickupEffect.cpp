...
PickupLootEffect::PickupLootEffect(mono::ParticleSystem* particle_system, mono::IEntityManager* entity_system)
    : m_particle_system(particle_system)
    , m_entity_system(entity_system)
{
    mono::Entity particle_entity = m_entity_system->CreateEntity("PickupLootEffect", { TRANSFORM_COMPONENT, PARTICLE_SYSTEM_COMPONENT });
    particle_system->SetPoolData(
        particle_entity.id,
        20,
        "res/textures/particles/glyph1.png",
...
PickupLootEffect::~PickupLootEffect()
{
    m_entity_system->ReleaseEntity(m_particle_entity);
}

void PickupLootEffect::EmitAt(const math::Vector& position)
{
    const auto particle_generator = [](const mono::ParticleGeneratorContext& context, mono::ParticlePoolComponentView& view) {
        ...
        const float magnitude_variation = mono::Random(5.0f, 7.0f);
        const float direction = mono::Random(0.0f, 360.0f);
        const math::Vector& velocity = math::VectorFromAngle(math::ToRadians(direction)) * magnitude_variation;
