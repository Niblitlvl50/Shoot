...
namespace
{
    ...
    {
        const float life = mono::Random(0.3f, 1.0f);
        const float radians = mono::Random(0.0f, math::TAU());
        const math::Vector offset = math::VectorFromAngle(radians) * mono::Random(0.0f, 0.2f);

        component_view.position = world_position + offset;
        component_view.velocity = math::Vector(0.0f, -0.1f);
        ...
        component_view.gradient = mono::Color::MakeGradient<4>(
            ...
            {
                mono::Color::MakeWithAlpha(DARK_GRAY, 0.0f),
                mono::Color::MakeWithAlpha(DARK_GRAY, 0.5f),
                mono::Color::MakeWithAlpha(DARK_GRAY, 0.3f),
                mono::Color::MakeWithAlpha(DARK_GRAY, 0.0f)
            }
        );
        component_view.size = 16.0f;
        component_view.start_size = 16.0f;
        component_view.end_size = 32.0f;
